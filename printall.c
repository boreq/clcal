...
/*************************************************************************/
/* return current year, month, or date as integers */
...
switch(iFormat)
   {
   case (0) :  strftime(szThisTime, 5, "%Y", pToday);
	       break;
   case (1) :  strftime(szThisTime, 5, "%m", pToday);
	       break;
   case (2) :  strftime(szThisTime, 5, "%d", pToday);
	       break;
   default  :  fprintf(stderr, "Error in getnow function.\n");
	       exit(EXIT_FAILURE);
   }

iThisTime = atoi(szThisTime);
return iThisTime;
